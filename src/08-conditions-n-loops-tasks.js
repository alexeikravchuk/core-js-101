...
function findFirstSingleChar(str) {
  ...
  let firstSingleCharIndex = str.length;

  str.split('').forEach((char, i) => {
    // eslint-disable-next-line no-unused-expressions
    chars[char] ? chars[char].counter += 1 : chars[char] = { counter: 1, index: i + 1 };
  });
  Object.keys(chars).forEach((char) => {
    if (chars[char].counter === 1 && chars[char].index < firstSingleCharIndex) {
      firstSingleChar = char;
      firstSingleCharIndex = chars[char].index;
    }
  });
